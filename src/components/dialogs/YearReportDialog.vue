...
onMounted(async () => {
  const YTDExpenses = await DB.getExpensesForYearCategory(props.year, 'All Categories')

  expenses.value = Number(
    YTDExpenses.reduce(
      (acc, expense) => acc + Number(expense?.amount ? expense.amount : 0),
      0
    ).toFixed(2)
  )

  housingTotal.value = Number(
    (await DB.getExpensesForYearCategory(props.year, ExpenseCategory.HOUSING))
      .reduce((acc, expense) => acc + Number(expense?.amount ? expense.amount : 0), 0)
      .toFixed(2)
  )
  transportationTotal.value = Number(
    (await DB.getExpensesForYearCategory(props.year, ExpenseCategory.TRANSPORTATION))
      .reduce((acc, expense) => acc + Number(expense?.amount ? expense.amount : 0), 0)
      .toFixed(2)
  )
  educationTotal.value = Number(
    (await DB.getExpensesForYearCategory(props.year, ExpenseCategory.EDUCATION))
      .reduce((acc, expense) => acc + Number(expense?.amount ? expense.amount : 0), 0)
      .toFixed(2)
  )
  groceriesTotal.value = Number(
    (await DB.getExpensesForYearCategory(props.year, ExpenseCategory.GROCERIES))
      .reduce((acc, expense) => acc + Number(expense?.amount ? expense.amount : 0), 0)
      .toFixed(2)
  )
  entertainmentTotal.value = Number(
    (await DB.getExpensesForYearCategory(props.year, ExpenseCategory.ENTERTAINMENT))
      .reduce((acc, expense) => acc + Number(expense?.amount ? expense.amount : 0), 0)
      .toFixed(2)
  )
  healthFitnessTotal.value = Number(
    (await DB.getExpensesForYearCategory(props.year, ExpenseCategory.HEALTH_FITNESS))
      .reduce((acc, expense) => acc + Number(expense?.amount ? expense.amount : 0), 0)
      .toFixed(2)
  )
  investmentsTotal.value = Number(
    (await DB.getExpensesForYearCategory(props.year, ExpenseCategory.INVESTMENTS))
      .reduce((acc, expense) => acc + Number(expense?.amount ? expense.amount : 0), 0)
      .toFixed(2)
  )
  eatingDrinkingOutTotal.value = Number(
    (await DB.getExpensesForYearCategory(props.year, ExpenseCategory.EATING_DRINKING_OUT))
      .reduce((acc, expense) => acc + Number(expense?.amount ? expense.amount : 0), 0)
      .toFixed(2)
  )
  giftsTotal.value = Number(
    (await DB.getExpensesForYearCategory(props.year, ExpenseCategory.GIFTS))
      .reduce((acc, expense) => acc + Number(expense?.amount ? expense.amount : 0), 0)
      .toFixed(2)
  )
  otherTotal.value = Number(
    (await DB.getExpensesForYearCategory(props.year, ExpenseCategory.OTHER))
      .reduce((acc, expense) => acc + Number(expense?.amount ? expense.amount : 0), 0)
      .toFixed(2)
  )
...
<template>
  ...
  >
    <QToolbar class="bg-info text-white" style="max-height: 50px">
      ...
      <QToolbarTitle>Report By Year</QToolbarTitle>
      <QBtn flat round :icon="Icon.CLOSE" @click="onDialogOK" />
    </QToolbar>

    <QCard class="q-dialog-plugin">
      <QCardSection>
        <p class="text-h6">Expenses - {{ year }}</p>
